...
Настройки берутся из константы DEFAULT_SETTINGS и создаются только если ещё не существуют
в базе данных, что позволяет не перезаписывать уже изменённые администратором значения.
"""
...
async def init_default_settings(session: AsyncSession) -> None:
    ...
    Не перезаписывает существующие настройки. Используется при первом запуске приложения
    для заполнения базы данных начальными значениями конфигурации.
