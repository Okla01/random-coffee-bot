...
Содержит функции для получения текущего времени в часовом поясе Москвы (МСК)
и приведения datetime объектов к aware-формату с информацией о часовом поясе.
Используется для консистентной работы со временем во всём приложении.
"""
...
def ensure_aware_msk(dt: datetime | None) -> datetime | None:
    ...
    конвертирует в МСК. Используется для нормализации времени из различных источников.
