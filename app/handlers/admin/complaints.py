...
async def cb_complaint_block(
    ...
    async with session_factory() as session:
        ...
        if await is_complaint_processed(complaint):
            ...
            admin_name = f"@{admin_user.username}" if admin_user and admin_user.username else f"ID:{complaint.reviewed_by}"
...
async def handle_warning_text(
    ...
    if not warning_text or len(warning_text.strip()) == 0:
        await message.answer("❌ Текст предупреждения не может быть пустым. Попробуйте ещё раз:")
        return
